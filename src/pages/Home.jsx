      ...
      {trigger ? (
        <div className="h-screen flex flex-col justify-center items-center text-center w-full bg-red-600">
          <div className="py-5 text-center">
            ...
            <h1 className="text-5xl flex items-center justify-center text-white font-bold">Emergency Detected!</h1>
          </div>
          <div className="flex flex-col gap-4 w-8/12">
            <button
              className="text-white border-white border px-3 py-4 rounded-full"
              onClick={() => {
                setTrigger(false);
                setIsRunning(false);
              }}
            ...
            <button className="flex gap-6 items-center bg-white rounded-full px-3 py-2" onClick={reportDanger}>
              <div className="flex flex-col items-center justify-center" id="spinning">
                <div className="relative w-12 h-12 border-4 border-red-500 border-t-transparent rounded-full animate-spin"></div>
                <div className="absolute text-xl font-bold text-red-500">{countdown > 0 ? countdown : '🚨'}</div>
              </div>
              Yes, proceed
            </button>
          </div>
        ...
        <div className="h-screen flex flex-col items-center text-center gap-9 m-5">
          <h1 className="text-4xl font-semibold mb-5 rounded-full bg-red-500 text-white p-5">🚨ReportSOS</h1>
          <h1 className="text-2xl font-medium w-9/12">Shake your phone to send an emergency alert, or tap the SOS button below.</h1>
          <button
            onClick={triggerSOS}
